...
const RiskPanel: React.FC<RiskPanelProps> = ({ data }) => {
    ...
    return (
        <div className={`civic-card flex flex-col gap-6 transition-all duration-700 ${isCriticalEscape ? 'border-red-600 border-2 animate-pulse' : ''}`}>
                    ...
                    <div key={m.label} className="risk-meter-container">
                        <div className="flex justify-between items-center">
                            ...
                            <span className="font-serif text-2xl font-black text-[var(--color-navy)]">
                                {m.val} <span className="text-xs font-sans font-bold text-[var(--color-charcoal)]/30">/ 100</span>
                            </span>
                        </div>
